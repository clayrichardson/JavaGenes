...
public double mutationStandardDeviation = 0.5;
public double mutationFrequency = 0.5;
...
private void setParameters() {
    ...
    breeder = new gov.nasa.javaGenes.core.BreederElitist(this,2);
  for(int i = intervalCrossoverFrequencyInterval.random(); i > 0; i--) {
    ...
    c.setExtension(extensionInterval.random());
  	childMakerProvider.add(c);
  }
  for(int i = intervalCrossoverFrequencyInterval.random(); i > 0; i--) {
  	childMakerProvider.add(new ChromosomeCrossover());
  }
  for(int i = arrayCrossoverFrequencyInterval.random(); i > 0; i--) {
  	childMakerProvider.add(new ChromosomeArrayCrossover());
  }
  for(int i = mutationTransmissionFrequencyInterval.random(); i > 0; i--) {
      ChromosomeMutation m = new ChromosomeMutation(alleles, mutationStandardDeviationInterval.random(), mutationFrequencyInterval.random());
      m.setLimitToOriginalInterval(false);
      childMakerProvider.add(m);
  }
