...
public ChildMakerProvider teacherChildMakers = new ChildMakerProvider();
public int numberOfTeachers;
public int newTeachersPerGeneration;
public AlleleTemplate teacherAlleles;
...
private void setParameters() {
  ...
  potential = new StillingerWeberPartialSiF(SiFAll.getBodiesNames(),factorInterval,exponentInterval,cutoffInterval);
  ...
  StudentFitnessFunction fitness = new StudentFitnessFunction();
  FitnessFunction f;
  ...
  teacherAlleles = BreederWithTeachers.createAlleleTemplate(fitness);
  for(int i = 0; i < 3; i++)
    teacherChildMakers.add(new ChromosomeIntervalCrossover(teacherAlleles));
  teacherChildMakers.add(new ChromosomeMutation(teacherAlleles, 1e-307, 0.5));

  breeder = new BreederWithTeachers(this,numberOfTeachers,newTeachersPerGeneration,teacherChildMakers);
