...
public int SSRcapacity = 5;
public int taskSSRuse = 1;
public int typicalTimeBetweenGroundStationDownload = 60*90;

private void setParameters() {
    populationSize = gov.nasa.javaGenes.simulatedAnnealing.Breeder.requiredPopulationSize();
    ...
    //scheduler.addPlacer(new EarliestFromRandomWindowPlacer());
    ...
    ChildMakerProviderWeighted c = new ChildMakerProviderWeighted();
    for(int i = 0; i < 4; i++)
	c.add(new ChangingWeightsObject(new PermutationOrderMutation(i+1),i,0));
    childMakerProvider = c;
    //childMakerProvider.add(new PositionCrossover());
    //childMakerProvider.add(new RandomChild());
    childMakerProvider.setFitnessFunction(fitnessFunction);
    breeder = new gov.nasa.javaGenes.simulatedAnnealing.Breeder(this,new gov.nasa.javaGenes.simulatedAnnealing.Accepter(100,10,1));
}
